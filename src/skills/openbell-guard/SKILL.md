...
현재 구현 상태는 Phase 4의 P4-08 시간·버킷 유틸리티 단계입니다. 플러그인 구조, 지표 계약 복사본, 최소 합성 fixture와 `run_openbell.py --bundle --output` 실행 입구가 있습니다. CLI는 번들의 허용 파일, 필수 파일, UTF-8, 파일 크기, `incident.json` 시간 창과 선택 `service-map.json` 구조를 검사한 뒤, 7개 민감정보 패턴을 마스킹한 `sanitized-bundle/`과 `sanitization-report.md`를 생성합니다. 이어서 마스킹된 `logs.jsonl`과 `metrics.csv`를 행 단위로 검증해 M-014 기준 `record-summary.json`을 만들고, 유효한 구간 내 행을 UTC 60초 버킷으로 정렬한 `bucket-summary.json`을 생성합니다. 다만 아직 지표를 계산하거나, 최종 `analysis.json`을 생성하지 않습니다.
...
- 현재 단계에서는 CLI 경로 점검, 번들 사전 검사, 민감정보 마스킹 작업본 생성, 행 단위 레코드 요약과 60초 버킷 요약 생성을 수행할 수 있습니다.
- 실행 예시는 다음과 같습니다.
...
- 성공하면 `openbell-cli-summary.json`, `sanitized-bundle/`, `sanitization-report.md`, `record-summary.json`, `bucket-summary.json`을 생성합니다. 이 파일들은 P4-08 사전 처리 결과이며 최종 분석 산출물인 `analysis.json`은 아닙니다.
- 사용자가 분석 실행을 요청하면, 아직 지표 계산기는 구현 전이며 다음 단계에서 기본 지표 계산을 순차 구현한다고 안내합니다.
